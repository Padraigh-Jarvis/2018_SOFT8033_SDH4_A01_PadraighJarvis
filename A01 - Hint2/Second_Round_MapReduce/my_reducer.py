...
def my_reduce(input_stream, total_petitions, output_stream):
    ...
    for line in fileContents:
        ...
        if lineContents[0] not in results:
            results[lineContents[0]]=int(lineContents[1])
        else:
            results[lineContents[0]]=results[lineContents[0]]+int(lineContents[1])
    for key in results:
        persentage = (results[key]/total_petitions)*100
        output=key+"\t("+str(results[key])+","+str(persentage)+"%)"+"\n"
